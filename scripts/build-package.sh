...
#   --cache-dir       shared SDK tarball cache directory (populated /
#                     consulted for every candidate; the sha256 against the
#                     official sums is still enforced on reuse)
#
# Environment:
...
#
# Artifacts (both are real builds of this package definition):
#   luci-theme-mint-<ver>          - the theme (arch: all / noarch)
#   luci-i18n-mint-zh-cn-<ver>     - the Simplified Chinese translation
#                                    (auto-generated by luci.mk from po/)
# The translation is a SEPARATE package per the official LuCI API; a release
# that ships only the theme leaves the theme strings untranslated, so both
# are collected and verified here.
...
setup_feeds() {
	...
	if [ -n "$trev" ] && [ -n "$thash" ]; then
		...
		# Same revision for the translation package, so the pair is versioned
		# together (luci.mk would otherwise derive PKG_PO_VERSION from the
		# LuCI feed's git state / file mtimes).
		sed -i "s/^PKG_PO_VERSION ?=.*/PKG_PO_VERSION := $tver/" "$THEME_FEED_DIR/Makefile"
	fi
...
configure_sdk() {
	...
	# The theme's zh-Hans translation is a separate package (official LuCI
	# API: luci.mk generates luci-i18n-mint-zh-cn from po/). It is HIDDEN in
	# menuconfig and defaults to n, so select it explicitly - a release that
	# ships only the theme leaves every theme string untranslated.
	set_config "$sdk" CONFIG_PACKAGE_luci-i18n-mint-zh-cn "CONFIG_PACKAGE_luci-i18n-mint-zh-cn=m"
	...
	for dep in luci-base curl rpcd ucode ucode-mod-html ucode-mod-uci \
		liblucihttp-ucode libubus libubox cgi-io luci-i18n-mint-zh-cn; do
		grep -qE "^CONFIG_PACKAGE_${dep}=[my]" "$sdk/.config" || {
			warn "package '${dep}' missing from .config - the SDK feed set or LuCI branch is incomplete"
			ok=0
		}
	done
	[ "$ok" = 1 ] || die "the SDK's feed set cannot satisfy the package selection (luci-base chain + theme i18n)"
...
OUT="dist" WORKDIR="build" ALLOW_LEGACY=0 LUCI_BRANCH="" JOBS="" CACHE_DIR=""
...
for cand in $CANDIDATES; do
	...
	GITHUB_OUTPUT= "$SCRIPT_DIR/get-openwrt-sdk.sh" \
		...
		--format "$FORMAT" \
		${CACHE_DIR:+--cache-dir "$CACHE_DIR"} >"$meta_file" || rc=$?
	if [ "$rc" -ne 0 ]; then
...
# The translation package is part of the release (official LuCI API keeps it
# separate from the theme). Both must be present - a release without the
# zh-Hans catalog ships an untranslated theme.
mapfile -t I18N_PKGS < <(find "$SDK_DIR/bin/packages" -type f \
	\( -name "luci-i18n-mint-zh-cn*.${FORMAT}" \) 2>/dev/null | sort)
...
[ "${#I18N_PKGS[@]}" -gt 0 ] || {
	( cd "$SDK_DIR/bin/packages" && find . -type f \( -name '*.apk' -o -name '*.ipk' \) | head -50 ) >&2 || true
	die "no luci-i18n-mint-zh-cn *.${FORMAT} produced by the build system - check CONFIG_PACKAGE_luci-i18n-mint-zh-cn (configure_sdk)"
}

SLUG="$(target_slug "$TARGET")"
LABEL="$USED_VERSION"

# Version string embedded in a built package filename, for either backend:
#   ipk: <name>_<version>_all.ipk
...
PKG_VERSION="$(pkg_version_of "${PKGS[0]}")"
I18N_VERSION="$(pkg_version_of "${I18N_PKGS[0]}")"

FINAL_NAME="luci-theme-mint-${RELEASE_VERSION}-${LABEL}-${SLUG}-all.${FORMAT}"
I18N_FINAL_NAME="luci-i18n-mint-zh-cn-${RELEASE_VERSION}-${LABEL}-${SLUG}-all.${FORMAT}"
cp -f "${PKGS[0]}" "$OUT/$FINAL_NAME"
cp -f "${I18N_PKGS[0]}" "$OUT/$I18N_FINAL_NAME"

log "package: $OUT/$FINAL_NAME"
log "package: $OUT/$I18N_FINAL_NAME"
...
# The arch recorded inside the package: ipk control says "all", the apk
# backend stamps arch-independent packages as "noarch" (same content).
if [ "$FORMAT" = apk ]; then PKG_ARCH="noarch"; else PKG_ARCH="all"; fi

# One buildinfo file per package, shipped next to it as evidence of how it
# was produced (make-release-notes.sh renders both into the release table).
emit_buildinfo() {
	local final="$1" name="$2" ver="$3"
	{
		printf 'package            : %s\n' "$final"
	...
	} > "$OUT/${final%.${FORMAT}}.buildinfo.txt"
}
emit_buildinfo "$FINAL_NAME" "luci-theme-mint" "$PKG_VERSION"
emit_buildinfo "$I18N_FINAL_NAME" "luci-i18n-mint-zh-cn" "$I18N_VERSION"
...
github_out package_arch "$PKG_ARCH"
github_out i18n_package_file "$OUT/$I18N_FINAL_NAME"
github_out i18n_package_name "$I18N_FINAL_NAME"
github_out i18n_package_version "$I18N_VERSION"
...
printf 'package_format=%s\n' "$FORMAT"
printf 'package_arch=%s\n' "$PKG_ARCH"
printf 'i18n_package_file=%s\n' "$OUT/$I18N_FINAL_NAME"
printf 'i18n_package_name=%s\n' "$I18N_FINAL_NAME"
