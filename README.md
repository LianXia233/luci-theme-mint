...
- **单层视觉容器架构**：全站只有一个视觉容器 `.mz-view`，统一绘制背景/边框/圆角/阴影；内部 Section / CBI / 表格 / 卡片全部透明扁平化，禁止 Card 套 Card。壁纸在最底层，每页只做一次模糊，避免创建层叠上下文困住下拉层。Action Bar 为独立功能层（底部 sticky、全出血）
- **分层 CSS 架构**：`cascade.css` 只做 `@import` 编排，规则按职责拆为 `compat / tokens / base / layout / navigation / container / components / wallpaper / background / dark / animations / responsive / login` 十三层。旧样式整体保留为兼容底座，新设计系统按级联顺序覆盖。全站静态资源带 `?v=<MINT_ASSET_REV>` 缓存戳，JS 模块同享一套缓存键
- **容器几何单一所有者**：`.mz-view` 的 width / max-width / padding / margin 只由 `layout.css` + `container.css` 配合 tokens 决定，两侧留白对称（`margin: … auto`），任何宽度都不出现单边空洞。宽度上限 `--mz-content-max` 为 1920px
- **四态角色背景系统**：每页默认背景是一套四态独立的角色立绘层——PC×亮色 Arona、PC×暗色 Plana、Mobile×亮色 / Mobile×暗色各一张独立 3:4 竖版构图。设备由 media query 判定、明暗由 `<html data-theme>` 判定，纯 CSS、首帧即正确，JS 不参与。用户配置的自定义壁纸优先于角色背景
- **登录页角色背景（Plana）**：登录页不套四态，用 `.mint-background.mz-login-char` 单立绘节点（亮暗同一角色）；桌面右锚、手机底部锚定全幅。只有「角色背景（默认）」与「显式选择的壁纸」两种状态
- 基于 CSS 变量（Design Tokens）的现代设计系统：色彩、间距、圆角、阴影、字体、层叠层级、玻璃参数、容器令牌、动效曲线
- **Glass & Layered 视觉语言**：不对称容器圆角、品牌色偏移阴影、节标题强调色条、侧栏毛玻璃、菜单线性 SVG 图标 + 折叠箭头
- 浅色 / 深色 / 跟随系统三种配色（默认跟随系统）。浅色为白色微透毛玻璃，深色为深灰毛玻璃（不再是纯色空底）
- **壁纸客户端缓存版本戳**：设置页每次应用 / 上传 / 删除都会递增 `mz-wp-ver`，所有壁纸 URL 追加 `_mzv` 参数并预加载解码后再绘制，切换壁纸不再命中旧缓存
- 服务端壁纸缓存：cron 每 5 分钟把随机图落到 `/luci-static/mint/wallpaper-<kind>.img`，后台优先使用本地缓存（可 304 复用）。「后台随机壁纸」开关只控制是否每次导航重新拉取远程随机图，关闭时本地缓存仍生效
- 壁纸与布局解耦：壁纸层为 body 伪元素并置于负 z-index，内容区无需抬升层级，弹出层永不被侧栏或卡片压住
- 侧栏导航从 LuCI 实时菜单树渲染（无硬编码菜单）；移动端抽屉式导航 + 遮罩
- 480px 至 1280px+ 全段响应式；移动端表格重排为卡片
- 总览页增强（仅 Status > Overview 加载）：端口状态、系统信息、DHCP/Wireless/UPnP 区块卡片化；不支持的值显示 N/A，绝不造假数据
- 无障碍：跳转链接、focus-visible、aria 标签、键盘可操作的登录页
- 无 CDN、无 Web 字体、无图标字体、无前端框架、无 jQuery
- 国际化就绪（英文 + 简体中文：`po/` 编译为独立翻译包随 Release 发布）
- LuCI 弹窗系统主题化；cbi 选项卡完整样式；桌面隐藏原生页面标题栏（原生 `<h2>` 已承担标题，指示器槽位拆为独立 `.mz-indicatorbar`）；cbi-dropdown 深度主题化（展开方向与定位交还 LuCI 组件 JS）
- 全站 8px 半透明细滚动条；接口页定制（区域头色条、悬停详情、玻璃卡片）
- 第三方应用设计变量桥接（`--brand/--surface/--text`，兼容 taygedo 等应用）
- **壁纸设置已拆分为独立包 `luci-app-mint-wallpaper`**：主题包只提供 UI 与模板，壁纸设置页、rpcd 后端、缓存 cron 与 UCI 配置归该包所有，可单独安装/升级/卸载。侧栏入口位于「系统 → Mint 壁纸」
...
产出**四个文件**：主题 `luci-theme-mint` 与简中翻译 `luci-i18n-mint-zh-cn`（官方 LuCI 规范将 `po/` 编译为独立翻译包，只装主题时界面为英文），各两种格式，成对发布、成对安装。
...
> 提示：SDK 的包格式由其自带配置决定（25.12+/snapshot 为 `.apk`，24.10 SDK 为 `.ipk`）。以上流程等价于仓库里的 `scripts/build-package.sh`，需要多目标/双格式时可直接用它。
...
- 设备与模式判定全部在 CSS 内完成，JS 不参与，**首帧即正确**；切换明暗只翻转 `data-theme`，两层立绘 opacity 交叉淡入（零请求、不闪白）
- 四态资源在 `theme/htdocs/luci-static/mint/images/`，共 6 张 `.webp`：4 张四态立绘 + 2 张三角网格（登录页复用同一批资源）
- 该层 `fixed` + 负 z-index + 不接收指针事件，不在文档流内、不可能被命中测试选中，任何断点横向溢出恒为 0
- 登录页图层不透明（渐变画在图层自身），随机壁纸缓存无任何路径能透到角色之上

### 用户壁纸（可选，覆盖角色层）

- **登录页默认不加载随机壁纸**：只有 `mode='custom'`（用户显式配置的图片或直链）才绘制；未配置时是角色背景（Plana）
- **管理页随机壁纸默认关闭**（`ui_random '0'`）：显式打开后才会在每次导航重新拉取远程随机图
- 配置了自定义壁纸后，角色层 `display:none`，容器透明度恢复原值
- 服务端无壁纸缓存（ucode 后端只读 UCI 配置 + 钳制数值），因此没有「刷新缓存」按钮
...
`MINT_ASSET_REV` 常量同时存在于 `header.ut`、`footer.ut`，以及 `cascade.css` 全部 `@import` 的 `?v=`。**改动 `htdocs/luci-static/mint/` 下任何文件（含 `images/`）都必须同步 bump 三处**，否则路由器长缓存头会让浏览器继续用旧样式。该戳同时经 `<meta name="mz-asset-rev">` 注入 `L.env.resource_version`，使主题自身 JS 模块也随主题升级刷新缓存。
...
| --- | --- | --- | --- | --- |
| OpenWrt | 23.05+ / main | ucode（`.ut`） | `.ipk` | 主线 23.05+ 已切换至 ucode 模板引擎；main / snapshot 持续跟进 |
| ImmortalWrt | 21.02+ | ucode | `.apk` | ImmortalWrt 早于主线迁移至 ucode，并默认使用 apk 包管理 |
| LEDE / OpenWrt ≤ 19.07 | — | — | — | **不支持**：ucode 模板与 rpcd ACL 路径在旧分支不可用 |

云编译产物在每次 Release 同时提供双格式：`.apk`（OpenWrt 25.12+ / ImmortalWrt）与 `.ipk`（仍使用 opkg 的 24.10 / 23.05），直接选择与你设备包管理器对应的产物安装；两种格式各含主题与简中翻译，共 4 个包。
...
| `luci-i18n-mint-zh-cn` | 本主题的简体中文翻译（官方 LuCI 规范的独立翻译包） | 需要中文界面时必需，与主题成对安装 |
